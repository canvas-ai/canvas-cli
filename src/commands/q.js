...
export class QCommand extends BaseCommand {
    ...
    async handleQuery(parsed, currentContext = null) {
        ...
        if (!query && !stdinData) {
            console.error(chalk.red('Error: Please provide a query or pipe data to analyze'));
            this.showHelp();
            return 1;
        }

        // Determine template based on options or data
        let templateName = this.options.template || this.config.get('ai.contextTemplate') || 'canvas-assistant';
    ...
    isCodeQuery(query) {
        const codeKeywords = [
            ...
            'error', 'exception', 'syntax', 'algorithm', 'refactor', 'optimize',
            'review', 'test', 'unit test', 'integration', 'api', 'database',
            'sql', 'javascript', 'python', 'java', 'typescript', 'react',
            'node', 'npm', 'git', 'repository', 'commit', 'branch', 'merge'
        ];
    ...
    async handleStatus(parsed) {
        try {
            ...
            for (const [name, info] of Object.entries(status)) {
                ...
                const statusText = info.available ? chalk.green('Available') : chalk.red('Unavailable');
    ...
    showHelp() {
        ...
        console.log('  --connector <name>    Use specific AI connector (anthropic, openai, ollama)');
        ...
        console.log('    canvas config set connectors.ollama.host http://localhost:11434');
        console.log();
        console.log(chalk.cyan('The AI assistant is context-aware and will include information'));
        console.log(chalk.cyan('about your current Canvas context and environment in queries.'));
    }
}
