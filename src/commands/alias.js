...
export class AliasCommand extends BaseCommand {
    ...
    async handleList(parsed) {
        try {
            ...
            await this.output(Object.entries(aliases).map(([alias, config]) => ({
                ...
                updatedAt: config.updatedAt ? new Date(config.updatedAt).toLocaleString() : 'Never'
            })), 'alias');
    ...
    async handleSet(parsed) {
        ...
        const reservedNames = ['remote', 'context', 'workspace', 'alias', 'auth', 'config', 'help'];
        ...
        if (resourceAddress.includes('@') && resourceAddress.includes(':')) {
            if (!isValidResourceAddress(resourceAddress)) {
                throw new Error('Invalid resource address format. Use: user@remote:resource[/path]');
            }
        }

        try {
            // Check if alias already exists
            const existingAlias = await this.apiClient.remoteStore.getAlias(aliasName);
            if (existingAlias && !parsed.options.force) {
                console.log(chalk.yellow(`Alias '${aliasName}' already exists and points to: ${existingAlias.address}`));
                console.log(chalk.yellow('Use --force to overwrite the existing alias'));
                return 1;
            }

            await this.apiClient.remoteStore.setAlias(aliasName, resourceAddress);

            console.log(chalk.green(`✓ Alias '${aliasName}' set to '${resourceAddress}'`));
            return 0;
        } catch (error) {
    ...
    async handleUpdate(parsed) {
        ...
        try {
            ...
            if (!existingAlias) {
                console.log(chalk.red(`Alias '${aliasName}' not found`));
                console.log(chalk.cyan(`Use 'canvas alias set ${aliasName} ${resourceAddress}' to create it`));
                return 1;
            }

            await this.apiClient.remoteStore.updateAlias(aliasName, resourceAddress);
            console.log(chalk.green(`✓ Alias '${aliasName}' updated to '${resourceAddress}'`));
            return 0;
        } catch (error) {
    ...
    showHelp() {
        ...
        console.log('  set <alias> <address>     Set an alias to a resource address');
        ...
        console.log('  canvas alias set dev-ctx john@dev.local:development-context');
        console.log('  canvas alias set my-ws user@remote:my-workspace');
        console.log('  canvas alias get prod');
        console.log('  canvas alias update prod admin@new-production.server:universe');
        ...
        console.log('    canvas context bind prod        # Instead of admin@production.server:universe');
        console.log('    canvas workspace show my-ws     # Instead of user@remote:my-workspace');
        console.log();
        console.log(chalk.bold('Notes:'));
        console.log('  • Aliases are stored locally in ~/.canvas/config/aliases.json');
        console.log('  • Alias names cannot conflict with command names');
        console.log('  • Use descriptive names for better organization');
        console.log('  • Aliases work with any Canvas resource (remotes, contexts, workspaces)');
    }
}
