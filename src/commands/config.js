...
export class ConfigCommand extends BaseCommand {
    ...
    async handleDelete(parsed) {
        ...
        if (!parsed.options.force) {
            console.log(chalk.yellow(`Warning: This will delete configuration key '${key}'.`));
    ...
    async handleValidate(parsed) {
        ...
        if (config.server?.auth?.type && !['token', 'jwt'].includes(config.server.auth.type)) {
    ...
    showHelp() {
        ...
        console.log('  show [key]            Show configuration (all or specific key)');
