...
export class AuthCommand extends BaseCommand {
  ...
  async handleLogin(parsed) {
    const email =
      parsed.args[1] || parsed.options.username || parsed.options.email;
    ...
    try {
      ...
      console.log(
        chalk.green(`✓ Successfully logged in as ${user.name || user.email}`),
      );
  ...
  async handleLogout(parsed) {
    try {
      ...
      console.log(
        chalk.yellow(
          "⚠ Logout may have failed on server, but local token cleared",
        ),
      );
      return 0;
    }
  ...
  async handleCreateToken(parsed) {
    ...
    try {
      ...
      console.log(
        chalk.green(`✓ API token '${token.name}' created successfully`),
      );
  ...
  async handleDeleteToken(parsed) {
    ...
    if (!parsed.options.force) {
      console.log(
        chalk.yellow(
          `Warning: This will permanently delete token '${tokenId}'.`,
        ),
      );
  ...
  async handleSetToken(parsed) {
    ...
    if (!token.startsWith("canvas-")) {
      throw new Error(
        'Invalid token format. Canvas tokens should start with "canvas-"',
      );
    }
    ...
    try {
      ...
      console.log(
        chalk.yellow("⚠ Warning: Token may be invalid or server unreachable"),
      );
    }
      ...
      try {
        ...
        if (authType === "jwt") {
          try {
            ...
            const profile = response.payload || response.data || response;
            console.log(
              `User: ${profile.name || profile.email} (${profile.email})`,
            );
          } catch (error) {
