...
import { getPlatformInfo, needsWindowsCompat } from '../utils/windows-compat.js';
...
export class DiagnosticCommand extends BaseCommand {
    ...
    async handlePlatform(parsed) {
        ...
        console.log(`  Is Bundled Executable: ${platformInfo.isBundled ? chalk.green('Yes') : chalk.yellow('No')}`);
        console.log(`  Is Bun Runtime: ${platformInfo.isBun ? chalk.green('Yes') : chalk.yellow('No')}`);
        ...
        console.log(`  Has Console: ${platformInfo.hasConsole ? chalk.green('Yes') : chalk.red('No')}`);
        console.log(`  Has Stdout: ${platformInfo.hasStdout ? chalk.green('Yes') : chalk.red('No')}`);
        console.log(`  Stdout is TTY: ${platformInfo.stdoutIsTTY ? chalk.green('Yes') : chalk.yellow('No')}`);
        console.log(`  Force Color: ${platformInfo.forceColor || 'Not Set'}`);
        console.log(`  Supports Color: ${chalk.supportsColor ? chalk.green('Yes') : chalk.red('No')}`);
        console.log();

        // Environment variables
        console.log(chalk.cyan('Environment Variables:'));
        const envVars = [
            'PATH', 'HOME', 'USERPROFILE', 'SHELL', 'TERM', 'COLORTERM',
            'FORCE_COLOR', 'NO_COLOR', 'DEBUG', 'NODE_ENV'
        ];
        ...
        if (platformInfo.isWindows) {
            ...
            console.log(`  Needs Windows Compatibility: ${needsWindowsCompat() ? chalk.yellow('Yes') : chalk.green('No')}`);
            console.log(`  Console Code Page: ${process.env.CHCP || 'Unknown'}`);
            console.log(`  Windows Terminal: ${process.env.WT_SESSION ? chalk.green('Yes') : chalk.yellow('No')}`);
            console.log(`  PowerShell: ${process.env.PSModulePath ? chalk.green('Yes') : chalk.yellow('No')}`);
            console.log();
        }
    ...
    async handleConfig(parsed) {
        ...
        try {
            ...
            console.log(`Config file exists: ${this.config.size > 0 ? chalk.green('Yes') : chalk.yellow('No')}`);
            console.log();
            ...
            if (deletedValue === undefined) {
                ...
                console.log(`  ✗ Delete test: Key still exists with value ${deletedValue}`);
            }
            ...
            console.log(chalk.red(`✗ Configuration system test failed: ${error.message}`));
            return 1;
        }
    ...
    showHelp() {
        ...
        console.log();
        console.log(chalk.cyan('Use these commands to troubleshoot platform-specific issues.'));
    }
}
