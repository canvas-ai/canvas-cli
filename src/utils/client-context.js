...
export class ClientContextCollector {
    ...
    generateFeatureArray() {
        ...
        // Desktop features
        if (env.desktop.session) features.push(`client/desktop/session/${env.desktop.session}`);
        if (env.desktop.currentDesktop) features.push(`client/desktop/environment/${env.desktop.currentDesktop}`);
        if (env.desktop.sessionType) features.push(`client/desktop/type/${env.desktop.sessionType}`);

        // Locale features
        if (env.locale.lang) features.push(`client/locale/lang/${env.locale.lang}`);
        if (env.locale.lcAll) features.push(`client/locale/all/${env.locale.lcAll}`);
