 ...
 */
export function constructResourceAddress(userIdentifier, remote, resource, path = '') {
...
export function extractResourceKey(address) {
    ...
    const remoteId = constructRemoteIdentifier(parsed.userIdentifier, parsed.remote);
...
export function determineResourceType(resource) {
    ...
    if (resource.includes('context') || resource.includes('ctx') || resource.match(/^ctx-/)) {
        return 'context';
    }
